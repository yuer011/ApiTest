...
from ApiTest import exceptions, logger, response, utils
from ApiTest.client import HttpSession
from ApiTest.context import SessionContext
...
class Runner(object):
    ...
    def _run_test(self, test_dict):
        ...
        if teardown_hooks:
            # add by lm09352
            self.session_context.update_test_variables("extra", extra)
        ...
        if extra.validators:
            from httprunner.extra import ExtraValidation
            extra_validation = ExtraValidation()
            try:
                extra_validation.extra_validate(extra.validators)
            except exceptions.ValidationFailure:
                raise
            finally:
                self.validation_results.extend(extra_validation.extra_validation_results)
